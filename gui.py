...
BG_DARK = "#2C3E50"         # Dark Blue/Gray Background (Container)
BG_PRIMARY = "#34495E"      # Slightly lighter background for main content
FG_LIGHT = "#ECF0F1"        # Light Text
ACCENT_BLUE = "#3498DB"     # Primary Accent Blue
SIDEBAR_COLOR = "#212F3C"   # Very dark color for the sidebar
SUCCESS_GREEN = "#2ECC71"   # Action/Success Green
WARNING_RED = "#E74C3C"     # Error/Logout Red
ADMIN_COLOR = "#F1C40F"     # Admin Gold/Yellow
...
class BankingApp(tk.Tk):
    # ... (init, show_frame, login, logout methods are unchanged) ...
    """
    ...
    def __init__(self, *args, **kwargs):
        ...
        style.map('T.TButton', background=[('active', ACCENT_BLUE)], foreground=[('active', FG_LIGHT)])

        # Action Button style (Green for transactions/confirmations)
        style.configure('Action.TButton', font=FONT_STYLE, padding=[15, 8], 
                        background=SUCCESS_GREEN, foreground=BG_DARK, borderwidth=0, relief="flat")
        
        # Logout/Admin Button styles
        style.configure('Logout.TButton', font=FONT_STYLE, padding=[15, 8], 
                        background=WARNING_RED, foreground=FG_LIGHT, borderwidth=0, relief="flat")
        style.configure('Admin.TButton', font=FONT_STYLE, padding=[15, 8], 
...
# ... (WelcomeFrame, RegisterFrame, LoginFrame, AdminLoginFrame are unchanged) ...
...
# ... (AccountSummaryFrame, DepositFrame, WithdrawFrame, TransferFrame, StatementFrame are unchanged) ...
